...
@Component({
    ...
    <mat-nav-list *ngFor="let node of treeData">
        <div (click)="toggleChildren(node)" style="width:calc(100% - 45px); float:left;">
            <a mat-list-item routerLink="{{node.target}}">
                {{node.title}}
            </a>
        </div>
        <div (click)="toggleChildren(node)" [ngClass]="{ has_item:node.children?.length}" style="float:right;width:41px;text-align:center;height: 32px; padding-top:4px;margin-top:4px;margin-bottom:8px">
